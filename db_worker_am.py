...
# ==========================================
# 2. AI 3인방 분석 엔진 (화폐단위 & 증권맨 패치)
# ==========================================
def run_plus_t_ai(ticker, name, market, data):
    # 💡 시장에 따른 화폐 단위 자동 설정
    currency = "달러($)" if market == "US" else "원(₩)"
    ...
    [데이터 시트]
    1. 가격/차트: 현재가 {data['current_price']}{currency}, 20일선 {data['ma20']}{currency}, 상단밴드 {data['bb_upper']}, 하단밴드 {data['bb_lower']}, RSI {data['rsi']}, 거래량강도 {data['vol_ratio']}배
    2. 실시간 뉴스: {news_str}

    [AI 비서별 역할 및 절대 금기사항]
    
    1. 🛡️ T-Shield (기술 지표 통합 분석가):
       - 임무: 차트를 '입체적'으로 분석하십시오. RSI, 20일선과의 이격도, 볼린저 밴드 내의 위치, 예상되는 '지지선'을 하나의 스토리로 엮어서 설명하세요.
       - 규칙: 가격을 언급할 때는 반드시 '{currency}' 단위를 붙이십시오. 뉴스 언급은 절대 금지합니다.
       
    2. 🐂 T-Bull (뉴스 및 커뮤니티 심리 전문가):
       - 임무: 제공된 '실시간 뉴스'와 '거래량 강도({data['vol_ratio']}배)'를 바탕으로, 현재 종토방이나 레딧 같은 온라인 커뮤니티의 대중 심리와 투심을 생생하게 묘사하세요.
       - 금기: 'RSI', '이평선', '밴드' 등 차트 숫자 언급 절대 금지.

    3. 🧠 T-Core (전략 통합 컨트롤러):
       - 임무: Shield의 차트 로직과 Bull의 커뮤니티 투심을 종합하여 상승 확률(win_rate)을 냉정하게 산출하고 결론을 내립니다.

    🚨 [말투 및 톤앤매너 절대 규칙 - 위반 시 즉시 해고]
    1. '재미있는 상태', '흥미로운 상황' 같은 기계적인 직역투를 절대 쓰지 마십시오.
    2. '변동성이 확대된 구간', '투심이 위축된 상태', '수급이 쏠리는 자리', '포모(FOMO) 장세' 등 한국 여의도 증권가나 토스증권 스타일의 전문적이고 세련된 금융 용어만 사용하십시오.
    3. '앱', '슬라이더', '서비스 운영' 등의 단어는 절대 금지합니다. 오직 주식 분석만 하십시오.
    ...
    {{
        ...
        "t_bull_opinion": "🐂 Bull: (뉴스+커뮤니티 투심 분석)",
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "system", "content": "너는 냉철하고 전문적인 한국의 엘리트 주식 분석가다. 직역투를 피하고 금융권 전문 용어를 자연스럽게 사용하라. 앱 개발자처럼 굴지 마라."},
                      {"role": "user", "content": prompt}],
            response_format={ "type": "json_object" },
            temperature=0.3 # 헛소리 차단 및 정석적인 분석 유도
        )
...
def main():
    ...
    print(f"🚀 Plus-T AI 엔진 가동 ({today_str}): 총 {len(targets)} 종목 입체 분석 시작!\n")
    ...
    for i, item in enumerate(targets):
        ...
        # 💡 market 인자를 추가로 넘겨줍니다.
        analysis = run_plus_t_ai(item['ticker'], item['name'], item['market'], data)
        ...
        try:
            supabase.table("tb_t_index").insert({
                ...
                "is_popular": True if i < 10 else False,
